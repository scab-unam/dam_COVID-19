...
p = {'N0':10**6, 'I0':np.array([0,1,2,0]),'W0':0,'transmissionGExposure':0.5,
...
def plotEpidemic(N,W,I,p,maxCases=10000,maxDeaths=1000):
    ...
    ax[0].set_xlabel('Days from first report on Feb 28, 2020')
    #dax.set_xlim(0,50); dax.set_ylim(0,maxDeaths)
    strTit= '''Covid-19 dynamics taking into account broad risk groups defined by clinical outcomes \n
    (Herrera-Nolasco, Herrera-Valdez, 2020)'''
    ax[0].text(peakInd+5, ITot.max()*.95, '%d days, %d infected'%(peakDay,ITot[peakInd]))
    ax[0].text(peakInd+5, ITot.max()*.9, 'Peak estimated around May 4, 2020')
...
p['offset']=-39
fig = plotEpidemic(N,W,I,p,maxCases=30000,maxDeaths=1000)
